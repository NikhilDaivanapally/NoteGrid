  ...
  endpoints: (builder) => ({
    ...
    createNote: builder.mutation<Note, Partial<Note>>({
      ...
      async onQueryStarted(body, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          api.util.updateQueryData("getNotes", DEFAULT_NOTE_QUERY, (draft) => {
            draft.data.unshift({
              ...body,
              _id: "temp-id",
              isFavorite: false,
    ...
    updateNote: builder.mutation<Note, Partial<Note>>({
      ...
      async onQueryStarted({ _id, ...body }, { dispatch, queryFulfilled }) {
        ...
        const patchSingle = dispatch(
          api.util.updateQueryData("getNoteById", _id as string, (draft) => {
    ...
    >({
      ...
      async onQueryStarted({ id, filters }, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          api.util.updateQueryData("getNotes", filters, (draft) => {
